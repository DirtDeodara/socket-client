...
const CommentContainer = ({ color, id, open, setHasNewComments }) => {
  const [reply, setReply] = useState("");
  const [comments, setComments] = useState(() => {
    const storedCommentList = localStorage.getItem(`${id}Comments`);
    return storedCommentList !== null ? JSON.parse(storedCommentList) : [];
  });
  ...
  const commentElements = comments.map(
    ({ user: commentAuthor, text: commentMessage }, i) => (
      <div className="comment" key={i}>
        ...
        <hr
          style={{
            border: "2px dashed",
            borderColor: colorFactory[color].mainBackground,
            borderRadius: 5,
          }}
        />
      </div>
  ...
  return (
    <div
      className={`${
        open ? "commentContainerOpen" : "commentContainerClosed"
      } shoutoutContainerPadding commentContainer`}
      style={{ backgroundColor: colorFactory[color].commentBackground }}
    >
      {commentElements}
      <ReplyInput
        setMessage={setReply}
        ...
        message={reply}
        buttonColor={colorFactory[color].accent}
      />
    </div>
  );
};

const Shoutout = ({ author, color, id, recipient, text, handleConfetti }) => {
  ...
  return (
    <div className="shoutoutOuterContainer">
      <div
        className={`${
          commentsOpen ? "flattenedBottomCorners" : "curvedBottomCorners"
        } shoutoutContainer`}
        style={{
          backgroundColor: colorFactory[color].mainBackground,
          color: colorFactory[color].text,
        }}
      >
        <div className="shoutoutContainerPadding">
          <CommentIcon
            alt="comment"
            className={`${
              commentsOpen ? "commentIconOpen" : "commentIconClosed"
            } ${hasNewComments && !hasBeenRead ? "pulse" : null } commentIcon`}
          ...
          <h2>
            ...
            <span
              style={{
                color: colorFactory[color].accent,
          ...
          <div className="emojiRow">
            ...
            />{" "}
            {/** currently applied to all emojis, but we could choose to have it one only one or two */}
            <Emoji
              id={id}
              label="love"
              symbol="❤️"
      ...
      <CommentContainer color={color} id={id} open={commentsOpen} setHasNewComments={setHasNewComments}/>
    </div>
  );
};
